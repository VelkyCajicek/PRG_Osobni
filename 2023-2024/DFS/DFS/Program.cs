...
namespace DFS
{
    internal class Program
    {
        ...
        static List<int> DepthFirstSearch(BinaryTree graph, int targetNodeIndex)
        {
            ...
            Node secondNode = currentNode.children[1];
            ...
            */
            ...
            for (int i = 0; i < currentNode.children.Count(); i++)
            {
                ...
                if(child.index == targetNodeIndex)
                {
                    pathToTargetNode.Add(child.index);
                    child = currentNode.parent;
                }
                else
                {
                    currentNode = child;

                    for (int j = 0; i < currentNode.children.Count(); i++)
                    {
                        Node childrenOfChildren = currentNode.children[j];
                        if (child.index == targetNodeIndex)
                        {
        ...
        static void Main(string[] args)
        {
            ...
            */
            Console.WriteLine();
            //Console.WriteLine(currentNode.parent.index);
